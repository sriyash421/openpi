...
Usage:
uv run examples/libero/convert_pathmask_libero_data_to_lerobot_test_vlm.py --data_dir /path/to/your/data --paths_masks_file /path/to/vlm_predictions.h5
            ...
            for libero_h5_file in libero_h5_list:
                with h5py.File(Path(data_dir) / raw_dataset_name / libero_h5_file, "r", swmr=True) as f:
                    ...
                    for demo_name in f["data"]:
                        ...
                        episode_key = f"{libero_h5_file.split('.')[0]}_{demo_name}"
                        if episode_key in path_masks_h5:
                            ...
                            path_lengths = (
                                episode_group["image_path_lengths"][:]
                                if "image_path_lengths" in episode_group
                                else None
                            )
                            mask_data = episode_group["image_masks"][:] if "image_masks" in episode_group else None
                            mask_lengths = (
                                episode_group["image_mask_lengths"][:]
                                if "image_mask_lengths" in episode_group
                                else None
                            )
                            path_timesteps = (
                        ...
                        for i in range(num_steps):
                            ...
                            if path_data is not None:
                                ...
                                if i == path_timesteps[
                                    ...
                                    current_path = path_data[
                                        next_path_timestep_idx, : path_lengths[next_path_timestep_idx]
                                    ]
                            ...
                            if use_subtask_instructions:
                                ...
                                if (
                                    current_subtask is not None and new_subtask != current_subtask
                                ) or i == num_steps - 1:
